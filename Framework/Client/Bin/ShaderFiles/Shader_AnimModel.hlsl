...
matrix GetAnimationMatrix_2(VS_IN input)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (animIndex[1] >= 0)
		{
			...
			float4 vSourQuat, vDestQuat;
			float3 vSourTranslate, vDestTranslate;
			
			decompose(result, vSourTranslate, vSourQuat, vSourScale);
			decompose(nextResult, vDestTranslate, vDestQuat, vDestScale);
			
			float3 vResultScale = lerp(vSourScale, vDestScale, g_TweenFrames.fTweenRatio);
			float4 vResultQuat = lerp(vSourQuat, vDestQuat, g_TweenFrames.fTweenRatio);
			float3 vResultTranslate = lerp(vSourTranslate, vDestTranslate, g_TweenFrames.fTweenRatio);
			
			result = compose(vResultTranslate, vResultQuat, vResultScale);
