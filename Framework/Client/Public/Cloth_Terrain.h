...
class CCloth_Terrain final : public CGameObject
{
protected:
	/* 원형을 생성할 때 */
	CCloth_Terrain(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	/* 사본을 생성할 때 */
	CCloth_Terrain(const CGameObject& rhs); /* 복사 생성자. */

	virtual ~CCloth_Terrain() = default;
...
public:
	static CCloth_Terrain* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
