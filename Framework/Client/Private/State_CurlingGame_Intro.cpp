...
void CState_CurlingGame_Intro::Enter_State(void* pArg)
{
	...
	if (FAILED(Set_Character_Transform()))
		return;
}
...
HRESULT CState_CurlingGame_Intro::Ready_Cameras()
{
...
HRESULT CState_CurlingGame_Intro::Set_Character_Transform()
{
	const Vec3 vLook = m_pManager->m_tStandardDesc.vStartLook;
	const Vec3 vRight = Vec3::Up.Cross(vLook);

	const _float fSideDelta = 3.f;
	...
	{
		if (0 == CCurlingGame_Manager::PARTICIPANT_PLAYER)
		{
			vStartPosition += vRight * fSideDelta;
		}
		else if (1 == CCurlingGame_Manager::PARTICIPANT_NPC)
		{
			vStartPosition += -vRight * fSideDelta;
		}
