...
CState_SwordMan_Neutral_Run::CState_SwordMan_Neutral_Run(CStateMachine* pMachine)
    : CState_Character(pMachine)
{
}

HRESULT CState_SwordMan_Neutral_Run::Initialize(const list<wstring>& AnimationList)
{
...
void CState_SwordMan_Neutral_Run::Tick_State(_float fTimeDelta)
{
	Input(fTimeDelta);

	GET_INSTANCE(CParticle_Manager)->Tick_Generate_Particle(&m_fEffectAcc, CUtils::Random_Float(1.f, 2.f), fTimeDelta, TEXT("Particle_Smoke"), m_pCharacter,
		_float3(0.f, 0.f, -0.1f));
}

void CState_SwordMan_Neutral_Run::Exit_State()
{

}

void CState_SwordMan_Neutral_Run::Input(_float fTimeDelta)
{
...
CState_SwordMan_Neutral_Run* CState_SwordMan_Neutral_Run::Create(CStateMachine* pStateMachine, const list<wstring>& AnimationList)
{
    CState_SwordMan_Neutral_Run* pInstance = new CState_SwordMan_Neutral_Run(pStateMachine);

    if (FAILED(pInstance->Initialize(AnimationList)))
    {
        MSG_BOX("Fail Create : CState_SwordMan_Neutral_Run");
...
void CState_SwordMan_Neutral_Run::Free()
{
    __super::Free();
}
