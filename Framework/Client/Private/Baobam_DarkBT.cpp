...
HRESULT CBaobam_DarkBT::Initialize_Prototype(CMonster* pOwner)
{
	__super::Initialize_Prototype(pOwner);

	m_pBaobam_Dark = dynamic_cast<CBaobam_Dark*>(m_tBTMonsterDesc.pOwner);
	...
	/* Dead 관련 */
	CBaobam_DarkNode_Dead* pDeadNode = CBaobam_DarkNode_Dead::Create(&m_tBTMonsterDesc, this);

	/* Hit 관련 */
	CBTNode_Select* pSel_Hit = CBTNode_Select::Create(this);
	CBaobam_DarkNode_Stun* pStunNode = CBaobam_DarkNode_Stun::Create(&m_tBTMonsterDesc, this);
	CBaobam_DarkNode_Hit* pHitNode = CBaobam_DarkNode_Hit::Create(&m_tBTMonsterDesc, this);

	/* Combat 관련 */
	CBTNode_Sequence* pSeq_Pattern = CBTNode_Sequence::Create(this);
	CBaobam_DarkNode_Attack1* pAtk1Node = CBaobam_DarkNode_Attack1::Create(&m_tBTMonsterDesc, this);
	CBaobam_DarkNode_Attack2* pAtk2Node = CBaobam_DarkNode_Attack2::Create(&m_tBTMonsterDesc, this);
	CBaobam_DarkNode_Skill1* pSkill1Node = CBaobam_DarkNode_Skill1::Create(&m_tBTMonsterDesc, this);
	CBaobam_DarkNode_Skill2* pSkill2Node = CBaobam_DarkNode_Skill2::Create(&m_tBTMonsterDesc, this);

	/* Chase 관련 */
	CBaobam_DarkNode_Chase* pChaseNode = CBaobam_DarkNode_Chase::Create(&m_tBTMonsterDesc, this);

	/* Return 관련 */
	//CBaobam_DarkNode_Return* pReturnNode = CBaobam_DarkNode_Return::Create(&m_tBTMonsterDesc, this);
	...
	vecAnimationName.push_back(TEXT("SKM_Baobam_Darkness.ao|BaoBam_Idle02"));
	CBaobam_DarkNode_Idle* pIdleNode = CBaobam_DarkNode_Idle::Create(&m_tBTMonsterDesc, this, vecAnimationName);
	CBaobam_DarkNode_Roaming* pRoamingNode = CBaobam_DarkNode_Roaming::Create(&m_tBTMonsterDesc, this, dynamic_cast<CMonster*>(m_tBTMonsterDesc.pOwner)->Get_RoamingArea());
...
_bool CBaobam_DarkBT::IsAtk()
{
	if (m_pBaobam_Dark->Get_Bools(CMonster::MONSTER_BOOLTYPE::MONBOOL_COMBAT) &&
		m_pBaobam_Dark->Get_Bools(CMonster::MONSTER_BOOLTYPE::MONBOOL_ATKAROUND) ||
...
CBaobam_DarkBT* CBaobam_DarkBT::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, CMonster* pOwner)
{
