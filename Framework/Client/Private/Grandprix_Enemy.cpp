...
CGrandprix_Enemy::CGrandprix_Enemy(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strObjectTag)
	: CGameObject(pDevice, pContext, strObjectTag, OBJ_TYPE::OBJ_GRANDPRIX_ENEMY)
{
}

CGrandprix_Enemy::CGrandprix_Enemy(const CGrandprix_Enemy& rhs)
	: CGameObject(rhs)
{
}

HRESULT CGrandprix_Enemy::Initialize_Prototype()
{
...
HRESULT CGrandprix_Enemy::Initialize(void* pArg)
{
...
void CGrandprix_Enemy::Tick(_float fTimeDelta)
{
...
void CGrandprix_Enemy::LateTick(_float fTimeDelta)
{
...
HRESULT CGrandprix_Enemy::Render_Instance_AnimModel(CShader* pInstancingShader, CVIBuffer_Instancing* pInstancingBuffer, const vector<_float4x4>& WorldMatrices, const vector<TWEEN_DESC>& TweenDesc, const vector<ANIMODEL_INSTANCE_DESC>& AnimModelDesc)
{
	__super::Render();
	...
	if (m_strObjectTag == TEXT("TreeGrandfa"))
	{
		for (_uint i = 0; i < iNumMeshes; ++i)
		{
			...
			iPassIndex = iPassIndex == 1 ? iPassIndex = 1 : iPassIndex = 4;
...
HRESULT CGrandprix_Enemy::Render_Instance_AnimModel_Shadow(CShader* pInstancingShader, CVIBuffer_Instancing* pInstancingBuffer, const vector<_float4x4>& WorldMatrices, const vector<TWEEN_DESC>& TweenDesc, const vector<ANIMODEL_INSTANCE_DESC>& AnimModelDesc)
{
	if (nullptr == pInstancingShader || nullptr == m_pTransformCom)
		return E_FAIL;
...
void CGrandprix_Enemy::Collision_Enter(const COLLISION_INFO& tInfo)
{
	__super::Collision_Enter(tInfo);
}

void CGrandprix_Enemy::Collision_Continue(const COLLISION_INFO& tInfo)
{
	__super::Collision_Continue(tInfo);
}

void CGrandprix_Enemy::Collision_Exit(const COLLISION_INFO& tInfo)
{
	__super::Collision_Exit(tInfo);
}

void CGrandprix_Enemy::Ground_Collision_Enter(PHYSX_GROUND_COLLISION_INFO tInfo)
{
	__super::Ground_Collision_Enter(tInfo);
}

HRESULT CGrandprix_Enemy::Ready_Components()
{
	// 모델은 자식 클래스에서
